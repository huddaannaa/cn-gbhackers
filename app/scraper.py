...
# ---------------- Config ----------------
CONFIG = {
    "information_source": "gbhacker_hacks_cvevulnerability",
...
def initial_walk_oldest_to_newest(sess, state:Dict[str,Any], seen:set)->int:
    """
    Start at last_page (oldest) and walk descending to 1 (newest).
    Progress is persisted via state['current_page'] so restarts resume.
    """
    last_page = detect_last_page(sess)
    cur = state.get("current_page")
    if not isinstance(cur, int) or cur < 1 or cur > last_page:
        cur = last_page
    total=0
    print(f"[INFO] Initial walk oldest→newest: start at page {cur}, last_page={last_page}")
